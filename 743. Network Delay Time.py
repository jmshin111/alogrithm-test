...
times = [[5,2,4],[5,4,2],[2,1,3],[1,4,3],[1,3,6],[3,4,2],[4,3,1],[4,2,1]]
n = 5
k = 5
...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        queue = []
        def bfs(distance:int,index:int):
            visited[index] = 0
            path = set()
            path.add(index)
            queue.append([index,path])

            while queue:
                next_node = queue.pop(0)

                for node in list(graph[next_node[0]]):


                    if node[0] in visited:
                       visited[node[0]] = min(visited[next_node[0]] + node[1], visited[node[0]])
                       #print(node, visited, graph,next_node[1])
                    else:

                       visited[node[0]] = visited[next_node[0]] + node[1]
                       #print(node, visited, graph,next_node[1])
                    if node[0] in next_node[1]:
                        continue
                    new_path = next_node[1].copy()
                    new_path.add(node[0])
                    queue.append([node[0],new_path])
