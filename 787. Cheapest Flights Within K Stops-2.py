    ...
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, K: int) -> int:
        ...
        while queue:
            ...
            for x, y in graph[next_node[0]]:
                ...
                if x not in visited:
                    visited[x] = [[next_node[0],y]]
                    queue.append([x, next_node[1]+1])
                else:
                    visited[x].append([next_node[0],y])
        ...
        while queue:
            ...
            if next_node not in visited:
                continue
            for i in visited[next_node]: #target, dist
                for j in node_dist[next_node]: #dist
                    dist = j + i[1]
                    if i[0] not in node_dist:
                        node_dist[i[0]] = [dist]
                    else:
                        node_dist[i[0]].append(dist)
                queue.append(i[0])


        return sorted(node_dist[src])[0]
