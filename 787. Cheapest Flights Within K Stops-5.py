...
class Solution:
    ...
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, K: int) -> int:
        ...
        visited_height, node_dist, h = {}, {}, []

        print(graph)
        heapq.heappush(h,(0, src, src )) # dist, from, to
        node_dist[src] = 0
        visited_height[src] = 1
        result = 99999
        visited = set()
        ...
        while h:
            ...
            if next_node[1] not in node_dist:
                continue
            ...
            if next_node[1]==src or next_node[2] not in visited_height:
                ...
                if visited_height[next_node[1]] +1 <= K+2:
                    ...
                    if next_node[2] == dst:
                        print('founded',node_dist[dst],node_dist)
                        rollback_flag = True
                        result = min(result, node_dist[dst])
                    elif visited_height[next_node[2]] < K+2:
                        for x, y in graph[next_node[2]]:
                            heapq.heappush(h, (y, next_node[2], x))
                    while h:
                        temp_next_node = heapq.heappop(h)

                        if temp_next_node[1] not in visited_height:
                            continue
                        else:
                            if visited_height[temp_next_node[1] ] < visited_height[next_node[2]]:
                                rollback_flag = True

                            else:
                                rollback_flag = False
                            heapq.heappush(h, temp_next_node)
                            break

                if h and rollback_flag:
                    temp_next_node = heapq.heappop(h)
                    temp_height = visited_height[temp_next_node[1]]+1

                    for i in list(visited_height):
                        if visited_height[i] >= temp_height:
