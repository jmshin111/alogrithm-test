...
class Solution:
    ...
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, K: int) -> int:
        ...
        def dfs(next_node, depth ,cost):
            #print(next_node, depth ,cost)

            if depth <= K+1 and next_node == dst:
                Solution.result = cost
                return True
            elif depth == K+1 and next_node != dst:
                return False

            h = []
            for x,y in graph[next_node]:
               heapq.heappush(h,(y,x))

            while h:
                next_node = heapq.heappop(h)

                if dfs(next_node[1], depth + 1, cost + next_node[0]):
                    return True
